...
let rec solve_safe lp =
  scale_problem lp;
  use_presolver lp true;
  try
    simplex lp;
    let col_primals = get_col_primals lp in
      true
  with
    | No_primal_feasible_solution -> 
	(* printf "There are unsatisfiable inequalities.@\n"; *)
	false

let rec solve_lp arrays pbounds lp =
    scale_problem lp;
    use_presolver lp true;
    if solve_safe lp then
      (print_string "Solved.\n";
       lp)
    else
      raise Unsatisfiable
...
(***  let ineqs' = List.filter (fun lc -> isnontrivial(lc)) ineqs in ***)
  ...
  let _ = debug "making a lp problem..." in
