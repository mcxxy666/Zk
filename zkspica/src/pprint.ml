...
let rec print_efelem el = 
  let print_atef a = match a with
    ...
    | AEchk(Pub,x,Emap(e)) -> ("Chk_pub ("^(print_exname x)^",["^(print_efelem e)^"] )")  
    | AEchk(Pr,x,Emap(e)) -> ("Chk_pr ("^(print_exname x)^",["^(print_efelem e)^"] )") 
    | AEchk(Pub,x,Evar(n)) -> ("Chk_pub ("^(print_exname x)^", e"^(string_of_int n)^")")
    | AEchk(Pr,x,Evar(n)) -> ("Chk_pr ("^(print_exname x)^", e"^(string_of_int n)^")")
...
let rec print_etype t = match t with
    ETUnit -> "()"
  | ETName(Pub,e1,e2) -> 
      if (e1=Emap([])&&e2=Emap([])) then
       ("Un")
      else
       ("NPub("^(print_effect e1)^","^(print_effect e2)^")")
  | ETName(Pr,e1,e2) -> ("NPr("^(print_effect e1)^","^(print_effect e2)^")")
...
let rec print_proc prec print_t p = match p with
  ...
  | In(m,x,t,pr) ->
	  ...
	  | None -> print_string x);
       if pr=Zero then () else (print_string ".\n";print_proc prec_in print_t pr)) | InS(m,x,t,pr) ->
      (print_string (print_message m);print_string "??";
       (match print_t with
	    Some f -> print_string ("("^x^":"^(f t)^")")
	  | None -> print_string x);
  ...
  | Par(pr1,pr2) ->
      (if prec>prec_par then print_string "(";
       print_proc prec_par print_t pr1;print_string " |\n";print_proc prec_par print_t pr2;
       if prec>prec_par then print_string ")")
  | Nu(x,t,pr) ->
      (print_string "(";
       (match print_t with
  ...
  | NuAsym(x,t,y,t2,pr) ->
      (print_string "(";
       (match print_t with
	    Some f -> print_string ("newAsym "^x^":"^(f t)^",\n\t"^y^":"^(f t2))
  ...
  | Case(m,x,t1,pr1,y,t2,pr2) ->
	  ...
	  | None -> print_string ("inl("^x^").\n\t\t"));
       print_proc prec_case print_t pr1;print_string "\n\tis ";
       (match print_t with
  ...
  | Split(m,x,t1,y,t2,pr) ->
      (if prec>prec_split then print_string "(";
       ...
       (match print_t with
	    Some f -> print_string ("("^x^":"^(f t1)^","^y^":"^(f t2)^") in \n")
