...
(*  usage *)
let p1 = BifurcationEquality (ZeroKnowledgeProof, Stop, Stop)
let p2 = VerificationBranch (ZeroKnowledgeProof, Stop, Stop)

let m1 = Process p1
let m2 = Process p2

(*  usage *)
let e1 = [create_end_effect "L"; create_check_effect "N1"]
let e2 = [create_end_effect "M"; create_check_effect "N2"]

let union_e1_e2 = union_effects e1 e2
let diff_e1_e2 = difference_effects e1 e2

(*  usage *)
let ch_un = create_channel_type Un
let key_int = create_key_type Int
let enc_private = create_encryption_type Private
